...
function EnhancedRaidFrames:ProcessIndicator(indicatorFrame, unit)
	...
	for _, auraName in pairs(self.auraStrings[i]) do
		...
		if foundAura and ((not self.db.profile[i].mineOnly and not self.db.profile[i].notMineOnly) or (self.db.profile[i].mineOnly and castBy == "player") or (self.db.profile[i].notMineOnly and castBy ~= "player")) then
			break
		end
	end
	...
	if foundAura and UnitIsConnected(unit) and not self.db.profile[i].missingOnly and (not self.db.profile[i].mineOnly or (self.db.profile[i].mineOnly and castBy == "player")) and (not self.db.profile[i].notMineOnly or (self.db.profile[i].notMineOnly and castBy ~= "player")) then
